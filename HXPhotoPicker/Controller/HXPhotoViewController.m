...
- (void)setupUI {
    ...
    UIImage *img = [UIImage hx_imageNamed:@"hx_faceu_cancel"];
//    [img imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    UIBarButtonItem *cancelItem = [[UIBarButtonItem alloc]initWithImage:img style:UIBarButtonItemStylePlain target:self action:@selector(didCancelClick)];
...
//    self.bgView.barStyle = manager.configuration.bottomViewBarStyle;
    ...
    if ([HXPhotoCommon photoCommon].isDark) {
        ...
        self.bgView.backgroundColor = self.barTintColor;
//        self.bgView.translucent = NO;
//        [self.bgView setShadowImage:[UIImage new] forToolbarPosition:UIBarPositionTop];
//        [self.bgView setBackgroundImage:[UIImage new] forToolbarPosition:UIBarPositionAny barMetrics:UIBarMetricsDefault];
        themeColor = self.manager.configuration.themeColor;
...
//        [self.doneBtn setTitle:[NSBundle hx_localizedStringForKey:@"导入"] forState:UIControlStateNormal];
        self.countLabel.text = [NSString stringWithFormat:@"%ld/%ld %@",(long)selectCount,(unsigned long)self.manager.configuration.maxNum,[NSBundle hx_localizedStringForKey:@"已选"]];
    }else {
...
//                    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%ld)",[NSBundle hx_localizedStringForKey:@"导入"],(long)selectCount,(long)maxCount] forState:UIControlStateNormal];
                    self.countLabel.text = [NSString stringWithFormat:@"%ld/%ld %@",(long)selectCount,(long)maxCount,[NSBundle hx_localizedStringForKey:@"已选"]];
                }else {
...
//                [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld/%lu)",[NSBundle hx_localizedStringForKey:@"导入"],(long)selectCount,(unsigned long)self.manager.configuration.maxNum] forState:UIControlStateNormal];
                self.countLabel.text = [NSString stringWithFormat:@"%ld/%ld %@",(long)selectCount,(unsigned long)self.manager.configuration.maxNum,[NSBundle hx_localizedStringForKey:@"已选"]];
            }
        }else {
//            [self.doneBtn setTitle:[NSString stringWithFormat:@"%@(%ld)",[NSBundle hx_localizedStringForKey:@"导入"],(long)selectCount] forState:UIControlStateNormal];
            self.countLabel.text = [NSString stringWithFormat:@"%ld %@",(long)selectCount,[NSBundle hx_localizedStringForKey:@"已选"]];
        }
    }
...
//    self.doneBtn.frame = CGRectMake(0, 0, 112, 42);
//    self.doneBtn.center = CGPointMake(self.doneBtn.center.x, 25);
    
    self.doneBtn.frame = CGRectMake(self.frame.size.width - 132, 10, 112, 42);
    self.countLabel.frame = CGRectMake(20, 10, self.frame.size.width - 162, 42);
//    [self changeDoneBtnFrame];
...
- (UIButton *)doneBtn {
    if (!_doneBtn) {
        ...
        [_doneBtn setTitle:[NSBundle hx_localizedStringForKey:@"导入"] forState:UIControlStateNormal];
