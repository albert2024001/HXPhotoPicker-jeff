...
- (void)setManager:(HXPhotoManager *)manager {
    _manager = manager;
    self.backgroundColor = manager.configuration.bottomViewBgColor;
...
- (void)setSelectCount:(NSInteger)selectCount {
    ...
    [self.doneBtn setTitle:[NSString stringWithFormat:@"%@%@",[NSBundle hx_localizedStringForKey:@"导入"], text] forState:UIControlStateNormal];
    [self changeDoneBtnFrame];
}
...
- (void)layoutSubviews {
    ...
    self.doneBtn.frame = CGRectMake(0, 0, 60, 42);
...
- (UIButton *)editBtn {
    if (!_editBtn) {
        ...
        [_editBtn setTitle:[NSBundle hx_localizedStringForKey:@""] forState:UIControlStateNormal];
